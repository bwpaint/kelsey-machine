// ─── KMS lead form handler (runtime-agnostic core) ───────────────────────────
// SINGLE SOURCE OF TRUTH for lead submission. Written against the Web-standard
// Request/Response API so the exact same code runs under:
//   - Astro's Node adapter at astro/src/pages/api/submit.ts   (xCloud, current)
//   - Vercel's serverless function at api/submit.ts           (legacy, pre-cutover)
//
// Do NOT fork this logic into either caller. This repo has twice had features
// built in the wrong one of two parallel codebases; both entry points are thin
// shims for a reason.
//
// This function is the permanent auth proxy between the frontend and the
// WebWize Connect Forms WordPress endpoint. The X-API-Key never touches the
// browser; the server holds it as an env var.
//
...
//
// Either way, the browser-facing URL (/api/submit) and the payload shape are
// unchanged — see client/src/lib/submitLead.ts.
//
// Security notes (2026-08-11 review):
//   - formType is allowlisted (ALLOWED_FORM_TYPES) instead of forwarded
...
//     entry or email.
//   - Rate limiting is NOT implemented here. Under Vercel this was impossible
//     (no shared state between invocations). Under the xCloud Node adapter the
//     process IS long-lived, so an in-process limiter is now feasible — but
//     the correct place is still the edge (OpenLiteSpeed / firewall rule on
//     /api/submit) so it survives a restart and can't be starved by the app.
//     Tracked as a follow-up, not done here. The honeypot only stops naive
//     bots; a targeted script that skips the hp field bypasses it by design.
...
// Behind xCloud/OpenLiteSpeed the app sees the proxy's address on the socket,
// so X-Forwarded-For is the only truthful source. `fallback` lets the Astro
// route pass its own `clientAddress` for the direct-connection case.
function clientIp(req: Request, fallback = ""): string {
  const fwd = req.headers.get("x-forwarded-for") || "";
  const first = fwd.split(",")[0]?.trim();
  return first || req.headers.get("x-real-ip") || fallback;
}

/**
 * Handle a lead submission. Returns a Response ready to hand straight back to
 * whichever runtime called us.
 *
 * @param req      the incoming request (must have a JSON body)
 * @param remoteIp optional direct-socket IP, used only if no forwarding headers
 */
export async function handleLead(req: Request, remoteIp = ""): Promise<Response> {
  let body: LeadBody;
  try {
  ...
  // attempts are visible in the server logs, but return a fake success
  ...
  if (wpBase && wpKey) {
    const endpoint = wpBase.replace(/\/+$/, "") + "/wp-json/webwize-forms/v1/submit";
    ...
    try {
      const resp = await fetch(endpoint, {
        ...
        headers: {
          ...
          "X-API-Key": wpKey,
          // Pass-through so the WP side sees the real client context, not ours.
          Referer: referer,
          "User-Agent": ua,
        },
        body: JSON.stringify({
      ...
      return json(
        {
          error:
            data.message || "We couldn't send your request just now. Please call 346-350-1464.",
        },
  ...
  const subject =
    `New ${formType} lead — ${name}` + (service !== "Not specified" ? ` (${service})` : "");
